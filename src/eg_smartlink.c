...
extern smt_proto_ops jd_proto_ops;
#endif
...
static int sc_scan_init(void)
{
    ...
    if((opmode != WIFI_MODE_STA_ONLY) &&(wifi_config_set_opmode(WIFI_MODE_STA_ONLY) < 0))
    {
...
static void  smt_get_proto_ops(uint8_t configmode)
{
...
wifi_smart_connect_status_t wifi_smart_connect_init (uint8_t configmode,const uint8_t *key, const uint8_t key_length, wifi_smart_connect_callback_t  callback)
{
	...
	if(b_init) {
        LOG_E(joylink_smt,"already init.\n");
        xSemaphoreGive(smt_config_mutex);
        return WIFI_SMART_CONNECTION_ERROR;
    }
	
	if(psmt_proto_ops == NULL)
	{
         smt_get_proto_ops(configmode);
         if(psmt_proto_ops == NULL)
         {
            LOG_E(joylink_smt, "ops is NULL.\n");
            goto fail;
         }
    }
    if (smt_rx_queue == NULL) {
...
wifi_smart_connect_status_t wifi_smart_connect_start (uint16_t timeout_seconds)
{
    ...
    wifi_smart_connection_status_t ret = WIFI_SMART_CONNECTION_OK;
	 ...
	 if(smt_handle != NULL) {
        smt_state = eTaskGetState(smt_handle);
        LOG_E(joylink_smt,"smtcn task state:%d, cannot create it\n", smt_state);
        xSemaphoreGive(smt_config_mutex);
        return WIFI_SMART_CONNECTION_ERROR;
    }
	...
	do{
	    ...
	    if (origin_opmode == WIFI_MODE_AP_ONLY) {
	        LOG_W(joylink_smt, "7687 in opmode:[%d], cannot continue smart connection\n",
	              origin_opmode);
...
int ejcloud_smart_connect(uint8_t  configmode)
{
...
int32_t ejcloud_smart_stop(void)
{
...
int32_t ejcloud_connection_disconnect_ap(void)
{
...
void EG_smartlink_process(unsigned char confgiMode)
{
